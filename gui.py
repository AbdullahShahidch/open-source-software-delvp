...
<<<<<<< HEAD
        ...
        self.length_spin = tk.Spinbox(
            self, from_=8, to=64, textvariable=self.gen_length, width=4,
            font=("Segoe UI", 9), bd=1, relief="solid"
        )
        self.length_spin.grid(row=1, column=4, sticky="w")

        self.generate_button = tk.Button(
            self, text="Generate", command=self._generate_password, font=("Segoe UI", 9)
        )
...
<<<<<<< HEAD
        ...
        self.copy_button = tk.Button(
            self, text="Copy", command=self._copy_password_to_clipboard, font=("Segoe UI", 10),
        )
        self.copy_button.grid(row=7, column=2, sticky="e")

        # Improved breach/output display with subtle border and padding
        self.breach_label = tk.Label(
            self, text="", bg="#ffffff", font=("Segoe UI", 10, "bold"), wraplength=420,
...
<<<<<<< HEAD
    ...
    def _copy_password_to_clipboard(self):
        ...
        if not pw:
            self.breach_label.config(text="Nothing to copy. Type or generate a password.", fg="#d93025")
            return
        try:
            self.clipboard_clear()
            self.clipboard_append(pw)
            self.breach_label.config(text="Password copied to clipboard.", fg="#555")
        except Exception:
...
<<<<<<< HEAD
        ...
        if not pw:
            self.min_req_label.config(text="Minimum requirements: —", fg="#555")
        elif not unmet:
            self.min_req_label.config(text="Minimum requirements: met ✅", fg="#188038")
        else:
            self.min_req_label.config(text=("Missing: " + ", ".join(unmet)), fg="#d93025")

        # Max possible entropy for this length (assuming 94-char set)
        if pw:
            max_possible = round(len(pw) * math.log2(94), 1)
            self.max_possible_label.config(text=f"Max possible entropy: {max_possible} bits")
        else:
